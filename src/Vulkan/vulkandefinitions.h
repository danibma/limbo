...
namespace limbo::rhi
{
	...
	inline VkImageUsageFlags vkImageUsageFlags(TextureUsage usages)
	{
		VkImageUsageFlags bits = 0;

		if ((usages & TextureUsage::Sampled) == TextureUsage::Sampled)
			bits |= VK_IMAGE_USAGE_SAMPLED_BIT;

		if ((usages & TextureUsage::Storage) == TextureUsage::Storage)
			bits |= VK_IMAGE_USAGE_STORAGE_BIT;

		if ((usages & TextureUsage::Transfer_Dst) == TextureUsage::Transfer_Dst)
			bits |= VK_IMAGE_USAGE_TRANSFER_DST_BIT;

		if ((usages & TextureUsage::Transfer_Src) == TextureUsage::Transfer_Src)
			bits |= VK_IMAGE_USAGE_TRANSFER_SRC_BIT;

		if ((usages & TextureUsage::Color_Attachment) == TextureUsage::Color_Attachment)
			bits |= VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;

		if ((usages & TextureUsage::Depth_Attachment) == TextureUsage::Depth_Attachment)
